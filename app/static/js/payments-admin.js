// static/js/payments-admin.js
// Consolidated loader + refund/dispute/reject display & workflow logic
// Replace your current static/js/payments-admin.js with this file.
...
(function () {
  ...
  // filter toolbar parent (we will create it if doesn't exist)
  const headerContainer = document.querySelector('.payments-container');
  ...
  // refund workflow container (created dynamically inside modal)
  ...
  window.__paymentsAdmin._activeSummaryFilter = 'filtered'; // keep track of active filter
  ...
  function extractLocation(payment) {
    if (!payment) return '—';
    const payer = payment.payer || {};
    const addr = payer.address || payer.shipping_address || (payment.shipping && payment.shipping.address) || null;
    const parts = [];
    if (addr) {
      ...
      if (addr.country || addr.country_code) parts.push(addr.country || addr.country_code);
      if (addr.postal_code) parts.push(addr.postal_code);
    }
    if (!parts.length && payment.order && (payment.order.customer_address || payment.order.customer_country || payment.order.country)) {
      const o = payment.order;
      if (o.customer_address) {
        if (o.customer_address.city) parts.push(o.customer_address.city);
        if (o.customer_address.region) parts.push(o.customer_address.region);
        if (o.customer_address.country) parts.push(o.customer_address.country);
      } else {
        if (o.customer_country) parts.push(o.customer_country);
        if (o.country) parts.push(o.country);
      }
    }
    if (!parts.length) {
      if (payer.country_code) parts.push(payer.country_code);
      else if (payment.payer_email) parts.push(payment.payer_email);
    }
  ...
  // --- Refunds & disputes detection & net amount calculation ---
  ...
  function getRefundDate(payment) {
    // attempt to find earliest refund date from raw_response._refunds or captures[].refunds[*].create_time
    try {
      const rr = parseRawResponse(payment.raw_response);
      if (!rr) return null;
      if (rr._refunds && Array.isArray(rr._refunds) && rr._refunds.length) {
        // pick the first refund date available
  ...
  function refundAgeDays(payment) {
    const refundDate = getRefundDate(payment);
    const created = getPaymentDate(payment);
    const now = new Date();
    if (refundDate) {
      const diff = now - refundDate;
      return Math.floor(diff / (24 * 3600 * 1000));
    }
    // If no refund date, consider age since created (for overdue detection)
    if (created) {
  ...
  function refundEligible(payment) {
    // Business rule: refunds should be done within 3 days and product unopened / seal intact.
    // On frontend we can only check timeframe; product condition must be enforced server-side.
    const created = getPaymentDate(payment);
  ...
  // --- Modal renderer (renders refund/dispute info) ---
  function clearTableBodies() {
    ...
    // remove refund workflow if present
  ...
  function renderDetails(payment) {
    ...
    // Render list of refund records (if available)
    try {
      const rr = parseRawResponse(payment.raw_response) || {};
      const refunds = rr._refunds || (rr.purchase_units && rr.purchase_units[0] && rr.purchase_units[0].payments && rr.purchase_units[0].payments.captures && rr.purchase_units[0].payments.captures[0] && rr.purchase_units[0].payments.captures[0].seller_receivable_breakdown && rr.purchase_units[0].payments.captures[0].refunds) || [];
  ...
  function renderItems(items, payment) {
    ...
    if (!items || !items.length) {
      ...
      itemsTotalEl.textContent = (payment && (payment.total_amount || payment.total || payment.amount)) ? fmtMoneyDisplay(getPaymentAmount(payment), payment.currency || 'USD') : '—';
      return;
    }
    ...
    itemsTotalEl.textContent = explicitTotal ? fmtMoneyDisplay(parseAmountCandidate(explicitTotal) || explicitTotal, payment && payment.currency) : fmtMoneyDisplay(runningTotal, payment && payment.currency);
  }
  ...
  // state: last displayed payment object (used by refund workflow)
  ...
  function showPayment(payment) {
    ...
    clearTableBodies();
    if (titleEl) titleEl.textContent = 'Payment details — ' + (payment.id || payment.payment_id || payment.paymentId || '');
  ...
  function buildRefundWorkflow(payment) {
    ...
    const header = document.createElement('div'); header.style.fontWeight = 800; header.textContent = 'Refund / Hold / Settle Options';
    refundWorkflowEl.appendChild(header);

    // Action radios: on hold, review, refund, rejected
    ...
    const actionOptions = [
      ...
      { id: 'act_reject', val: 'rejected', label: 'Rejected / Settled (no refund — funds retained by WPerfumes)' }
    ];
    ...
    actionOptions.forEach(opt => {
      ...
      r.value = opt.val;
      r.id = opt.id;
      if (opt.val === 'refund') r.checked = true; // default to refund
    ...
    // Refund amount controls (visible only when 'refund' selected)
    ...
    confirmBtn.addEventListener('click', async (ev) => {
      ...
      if (!payload.payment_id) {
        alert('Cannot determine payment id for refund.');
        return;
      }

      // Safety: For 'rejected' action, require an extra confirmation (simple confirm dialog)
      if (payload.action === 'rejected') {
        const ok = confirm('Are you sure you want to mark this dispute as REJECTED/SETTLED (no refund)? This will keep funds as sales.');
        if (!ok) return;
      }
      ...
      try {
        ...
        // Refresh payments table / details: attempt to fetch fresh payment data from server
        ...
        // close workflow but keep modal open with updated details
        setTimeout(() => {
          if (refundWorkflowEl && refundWorkflowEl.parentNode) refundWorkflowEl.parentNode.removeChild(refundWorkflowEl);
          refundWorkflowEl = null;
          // re-open current payment details (if returned updated payment in js, try to show it)
          if (js && js.updated_payment) {
            showPayment(js.updated_payment);
          } else {
            // attempt to find payment in __paymentsAdmin.latestPayments
            try {
        ...
        alert('Refund action failed: ' + (err && err.message ? err.message : String(err)));
      } finally {
  ...
  // attach refundBtn handler (when modal open)
  function wireRefundButton() {
    ...
    refundBtn.addEventListener('click', (e) => {
      ...
      // insert workflow UI after details area inside modal
  ...
  // wire once when DOM ready
  document.addEventListener('DOMContentLoaded', wireRefundButton);
  // also call after modal is shown / scripts loaded
  wireRefundButton();
  ...
  function renderPaymentRow(payment) {
    ...
    // determine refund/dispute flags & classes
    ...
    const refundDays = eligibility.days;
    ...
    // add dataset attributes to help filtering
    try {
    ...
    // columns (ID, Provider, Order ID+product, Capture ID, Amount, PayPal Fee, Status, Payer, Email, Created, Actions)
    ...
    if (productTitle) {
      ...
      // append refund/dispute badge inline
      ...
      // if no product title but refunded/disputed badges
  ...
  async function fetchPayments(page = 1, per_page = 25) {
    try {
      ...
      if (window.__paymentsAdmin && window.__paymentsAdmin.duration) {
        ...
        if (d.type && d.type !== 'daily') url.searchParams.set('duration', d.type);
        if (d.type === 'custom' && d.from) url.searchParams.set('from', d.from);
        if (d.type === 'custom' && d.to) url.searchParams.set('to', d.to);
      }
  ...
  // --- Summaries: compute and UI update (net amounts) ---
  ...
  // --- Filtering behavior for summary cards ---
  ...
  function applyCategoryFilter(category) {
    // category: filtered, cash-in, prev-day, today, refunded, disputed
    ...
    const yesterdayEnd = new Date(todayStart.getTime() - 1);
    for (const r of rows) {
      // skip header-like rows (if any) or placeholder
      if (!r.dataset) {
        r.style.display = '';
        continue;
      }
  ...
  // attach click handlers to summary cards (if present)
  function wireSummaryCardClicks() {
    ...
    mapping.forEach(m => {
      ...
      if (!el) return;
      // ensure clickable area (card wrapper)
  ...
  durationSelect && durationSelect.addEventListener('change', (e) => {
    ...
    document.dispatchEvent(new CustomEvent('payments:durationChange', { detail: { duration: window.__paymentsAdmin.duration } }));
  });
  ...
  // listen for external renderRow filler events
  document.addEventListener('payments:renderRow', (e) => {
    try {
      ...
      // Ensure dataset attributes exist if external filler touched
      try {
        const refundedAmount = getRefundedAmount(payment);
        const disputedAmount = getDisputedAmount(payment);
        const net = getNetAmount(payment);
